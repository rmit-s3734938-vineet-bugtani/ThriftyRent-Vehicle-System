...
public class DateTime {
	...
	// converting string input for date by user to DateTime type
	public static DateTime stringToDateTime(String dateTime) {
		String[] dateTimeArray = dateTime.split("/");
		int date = Integer.parseInt(dateTimeArray[0]);
		int month = Integer.parseInt(dateTimeArray[1]);
		int year = Integer.parseInt(dateTimeArray[2]);
		return new DateTime(date, month, year);
	}

	// forwarding a date by a specific number of days
	public static DateTime getEstimatedReturnDate(String dt, int n) {
		SimpleDateFormat sdf = new SimpleDateFormat("dd/MM/yyyy");
